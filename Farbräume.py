...
if __name__ == '__main__':
    ...
    # resize
    img = cv2.resize(src=img, dsize=None, fx=.5, fy=.5, interpolation=cv2.INTER_NEAREST)
    mask = cv2.resize(src=mask, dsize=None, fx=.5, fy=.5, interpolation=cv2.INTER_NEAREST)
    ...
    cv2.rectangle(img, (corners[0], corners[1]), (corners[2], corners[3]), (0, 0, 255), thickness=1)  # draw a rectangle 10x10 pixels in the center of image
    ...
    cv2.createTrackbar('H', 'yoshi', 100, 180, nothing)

    # resize
    yoshi = cv2.resize(src=yoshi, dsize=None, fx=.5, fy=.5, interpolation=cv2.INTER_NEAREST)
    while True:
